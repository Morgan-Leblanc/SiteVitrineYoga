import React from "react";
import { Helmet } from "react-helmet";
import "../screensCss/YogaMethodsAndBenefits.css"

const YogaBenefits = () => {
  return (
    <>
      <Helmet>
        <title>Les vertus du yoga - Bienfaits physiques et mentaux</title>
        <meta name="description" content="Découvrez les nombreuses vertus du yoga : renforcement musculaire, souplesse, équilibre du système nerveux, apaisement mental et émotionnel, et développement de la pleine conscience." />
        <meta name="keywords" content="yoga, vertus, bienfaits, corps, esprit, méditation, pleine conscience, relaxation" />
        <link rel="canonical" href="https://votresite.com/yoga-vertus" />
      </Helmet>
      <article className="multipleDesignContainer">
        <h1 title="la méthode deGasquet">Les vertus du yoga</h1>
        <section>
          <p>
            Sagesse millénaire issue de la Tradition de l'Inde, Art de vivre juste,
            chemin de « bien Être » et d'éveil, le yoga est une Voie royale pour
            progresser vers soi-même en cultivant la Présence, la joie, la
            gratitude, l'ouverture du cœur …{" "}
          </p>
          <blockquote id="citationGlobal">
            « Le corps est le chantier de l'âme où l'esprit vient faire ses gammes »
            <br></br>Hildegarde de Bingen
          </blockquote>
          <p>
            Le yoga est une quête qui demande patience et persévérance. <br></br>Elle
            implique une progression qui va s'inscrire dans la durée.
          </p>
          <blockquote id="citationGlobal">
            « Celui qui déplace la montagne, c'est celui qui commence par enlever
            les petites pierres. » <br></br>Confucius
          </blockquote>
        </section>
        <section>
          <h1 title=" les vertus du yoga"> DU CORPS VERS L'ESPRIT, LES BIENFAITS DE LA PRATIQUE DU YOGA :</h1>
          <p>Redonner de la longueur et de la tonicité à ses muscles,<br></br>renforcer ses articulations et goûter une amplitude articulaire oubliée.</p>
          <p>Dénouer les tensions musculaires, stimuler les différents organes et toutes les fonctions du corps,<br></br> équilibrer le système nerveux…</p>
          <p>Retrouver une liberté de mouvement, aisance et fluidité en déliant le corps avec douceur et précision, <br></br> de manière progressive  et respectueuse du corps de chacun.</p>
          <p>Mobiliser et habiter en conscience ce corps que l'on est, dans le respect <br></br> et la bienveillance envers soi-même.</p>
          <p>Améliorer sa vitalité grâce à une meilleure circulation de l'énergie dans le corps.</p>
          <p>Renouer avec sa respiration et harmoniser avec bonheur le geste et le souffle dans sa plénitude.</p>
          <p>Apprendre à se recentrer et à endiguer le flot permanent et envahissant des pensées.<br></br> Le yoga est un chemin de pacification intérieure, d'apaisement du mental et de l'émotionnel, <br></br> une voie d'unification.</p>
          <p>Entrer en amitié avec soi-même, dans une relation bienveillante, tendre, apaisée.</p>
          <p>Développer notre capacité à se rendre disponible, ouvert,  à nous poser et <br></br> à laisser quelque chose en nous se poser.</p>
          <blockquote id="citationGlobal">
            « Devient en toi-même une maison de paix, un point tranquille tourné
            vers l'Etre essentiel, <br></br> vers la grande Vie » <br></br>KG Dürckeim
          </blockquote>
          <p>
            Donner la pleine attention au moment présent tel qu'il est et goûter son
            infinie richesse,<br></br> savourer la Vie qui nous traverse.
          </p>
          <blockquote id="citationGlobal">
            « La pleine conscience est la conscience sans jugement de chaque
            instant, <br></br> que l'on cultive en prêtant attention. » <br></br>Jon Kabat
            Zinn
          </blockquote>
          <p>Plonger au plus profond de soi et y trouver un espace de quiétude.</p>
          <p>Prendre le temps de la croissance intérieure. </p>
          <blockquote id="citationGlobal">
            « Qui veut trouver des perles ne doit pas se contenter de regarder la
            mer, il doit plonger » <br></br>Rumi
          </blockquote>
          <p>
            Goûter le tout dans la joie, le contentement et laisser fleurir la
            gratitude.
          </p>
          <blockquote id="citationGlobal">
            « Faisons de chaque moment qu'il nous est permis de vivre un moment de
            joie » <br></br>Joseph Wresinski
          </blockquote>
          <p>Puis laissez le yoga porter ses fruits dans notre vie</p>
          <blockquote id="citationGlobal">
            « Il y a un continuum entre ce que l'on vit sur le tapis et ce que l'on
            vit dans le quotidien » <br></br>Isabelle Morin Larbey
          </blockquote>
        </section>
      </article>
    </>
  );
};

export default YogaBenefits;
